    ...
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                for u in data:
                    defaults = {'xp': 0, 'level': 1, 'egitim_duzeyi': 'Üniversite', 'ana_hedef': 'Mühendis', 'sinavlar': [], 'chat_history': [], 'notes': [], 'dil': 'TR'}
...
if menu in ["🏠 Panel", "🏠 Dashboard"]:
    # DİNAMİK BAŞLIK: Meslek + Kullanıcı Adı
    ...
    if not u_info['data'].empty:
        ...
        with c2:
            ...
            st.plotly_chart(go.Figure(go.Pie(labels=['Tamamlanan', 'Kalan'], values=[ty, max(0, th-ty)], hole=.6)), use_container_width=True)
    ...
    for g in gunler:
        with st.expander(f"📅 {g.upper()}"):
            ...
            for idx, row in temp.iterrows():
                ...
                cc1.write(row['Görev'])
                y_v = cc2.number_input("Yapılan", value=int(row['Yapılan']), key=f"y_{idx}")
                if y_v != row['Yapılan']:
    ...
    with st.popover("💬 Mentorla Konuş"):
        ...
        if p_m:
            ...
            try:
                ...
                veritabanini_kaydet(st.session_state.db); st.rerun()
            except: st.warning("Bağlantı hatası.")
    ...
    if st.button("BAŞLAT"): st.session_state.pomo_calisiyor = True; st.session_state.son_guncelleme = time.time(); st.rerun()
    if st.session_state.pomo_calisiyor:
        ...
        if st.session_state.pomo_kalan_saniye <= 0:
            st.session_state.pomo_calisiyor = False; st.balloons(); u_info['xp'] += 30; veritabanini_kaydet(st.session_state.db)
    ...
    with st.container(border=True):
        st.subheader("👤 Profil Bilgilerini Güncelle")
        yeni_id = st.text_input("Kullanıcı Adı", value=u_id)
        yeni_sifre = st.text_input("Yeni Şifre", value=u_info['password'], type="password")
        yeni_meslek = st.text_input("Meslek / Kariyer Hedefi", value=u_info.get('ana_hedef', 'Mühendis'))
        
        if st.button("TÜMÜNÜ KAYDET"):
            # Kullanıcı adı değişikliği varsa veritabanını güncelle
            if yeni_id != u_id:
                st.session_state.db[yeni_id] = st.session_state.db.pop(u_id)
                st.session_state.aktif_kullanici = yeni_id
                u_id = yeni_id
            
            st.session_state.db[u_id]['password'] = yeni_sifre
            st.session_state.db[u_id]['ana_hedef'] = yeni_meslek
            veritabanini_kaydet(st.session_state.db)
            st.success("Bilgileriniz başarıyla güncellendi! Panelde mesleğiniz görünecek.")
            st.rerun()
