...
if st.session_state.pomo_calisiyor:
    ...
    if st.session_state.pomo_kalan_saniye <= 0:
        ...
        if 'aktif_kullanici' in st.session_state:
            ...
            st.session_state.db[u]['xp'] += 30;
            st.session_state.db[u]['pomo_count'] += 1
...
if 'aktif_kullanici' not in st.session_state or st.session_state.aktif_kullanici is None:
    ...
    with t2:
        ...
        if st.button("KAYDOL"):
            st.session_state.db[nu] = {'password': np, 'xp': 0, 'level': 1, 'egitim_duzeyi': 'Üniversite',
                                       'ana_hedef': 'Gelişim', 'dil': 'TR',
...
st.markdown(
    f"<style>h1, h2, h3 {{ color: {tema_rengi} !important; }} div.stButton > button:first-child {{ background-color: {tema_rengi}; color: white; }}</style>",
...
if menu in ["🏠 Panel", "🏠 Dashboard"]:
        ...
        with c1:
            fig = go.Figure([go.Bar(x=u_info['data']['Görev'], y=u_info['data']['Hedef'], name=L["labels"]["hedef"],
                             ...
                             go.Bar(x=u_info['data']['Görev'], y=u_info['data']['Yapılan'], name=L["labels"]["yapilan"],
                                    marker_color=tema_rengi)])
            fig.update_layout(height=250, barmode='group', margin=dict(l=0, r=0, t=0, b=0));
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            ty, th = u_info['data']['Yapılan'].sum(), u_info['data']['Hedef'].sum()
            st.plotly_chart(go.Figure(
                go.Pie(labels=[L['labels']['yapilan'], 'Kalan'], values=[ty, max(0, th - ty)], hole=.6,
                       marker_colors=[tema_rengi, '#FF5252'])).update_layout(height=250, showlegend=False),
    ...
    for i, g in enumerate(gunler):
        with cols[i]:
            st.markdown(
                f"<div style='background:{tema_rengi}; color:white; text-align:center; border-radius:5px; font-weight:bold; padding:5px;'>{g[:3].upper()}</div>",
                unsafe_allow_html=True)
    ...
    for g in gunler:
        with st.expander(f"📅 {g.upper()}"):
            ...
            for idx, row in temp.iterrows():
                ...
                if y_v != row['Yapılan']:
                    u_info['xp'] += 10
                    if u_info['xp'] >= (u_info['level'] * 200):
                        u_info['level'] += 1;
                        st.balloons()
                        st.toast(f"🎉 {L['labels']['rutbe']}: {mevcut_lakap_getir(u_info['level'], u_info['dil'])}!")
                    u_info['data'].at[idx, 'Yapılan'] = y_v;
                    veritabanini_kaydet(st.session_state.db);
                    st.rerun()
                if cc3.button("🗑️", key=f"d_{g}_{idx}"): u_info['data'] = u_info['data'].drop(idx); veritabanini_kaydet(
                    st.session_state.db); st.rerun()
            with st.form(f"f_{g}", clear_on_submit=True):
                f1, f2, f3 = st.columns([2, 1, 1])
                ng, nh, nb = f1.text_input(L['labels']['gorev']), f2.number_input(L['labels']['hedef'],
                                                                                  1), f3.selectbox(L['labels']['birim'],
                                                                                                   ["Konu", "Soru",
                                                                                                    "Sayfa", "Saat"])
                if st.form_submit_button(L["butonlar"]["ekle"]):
                    u_info['data'] = pd.concat([u_info['data'], pd.DataFrame(
                        [{'Gün': g, 'Görev': ng, 'Hedef': nh, 'Birim': nb, 'Yapılan': 0}])], ignore_index=True)
                    veritabanini_kaydet(st.session_state.db);
                    st.rerun()
    ...
    with st.form("ms"):
        c1, c2 = st.columns(2);
        d, t = c1.text_input("Ders"), c2.date_input("Tarih")
        if st.form_submit_button("Ekle"):
            u_info['sinavlar'].append({'ders': d, 'tarih': t.strftime("%d.%m.%Y")});
            veritabanini_kaydet(st.session_state.db);
            st.rerun()
    ...
    if c1.button(L["butonlar"]["baslat"],
                 use_container_width=True): st.session_state.pomo_calisiyor = True; st.session_state.son_guncelleme = time.time(); st.rerun()
    if c2.button(L["butonlar"]["durdur"], use_container_width=True): st.session_state.pomo_calisiyor = False; st.rerun()
    if c3.button(L["butonlar"]["sifirla"],
                 use_container_width=True): st.session_state.pomo_calisiyor = False; st.session_state.pomo_kalan_saniye = 25 * 60; st.rerun()
    ...
    k1.metric(L["labels"]["rutbe"], su_anki_lakap)
    ...
    st.divider();
    st.subheader("🏅 Koleksiyonun")
    b1, b2, b3 = st.columns(3)
    if u_info.get('pomo_count', 0) >= 10:
        b1.success("🔥 ODAK USTASI")
    else:
        b1.info(f"🔒 ODAK USTASI ({u_info.get('pomo_count', 0)}/10)")
    if u_info['level'] >= 10:
        b2.success("👑 VİZYONER")
    else:
        b2.info("🔒 VİZYONER (Lvl 10)")
    tp = u_info['data']['Yapılan'].sum() if not u_info['data'].empty else 0
    if tp >= 100:
        b3.success("💎 ELMAS")
    else:
        b3.info(f"🔒 ELMAS ({tp}/100)")
