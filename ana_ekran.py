...
with t1:  
    ...
    if st.button("SİSTEME GİR"):  
        if (  
            u_l in st.session_state.db  
            and st.session_state.db[u_l].get("password") == p_l  
        ):  
            st.session_state.user = u_l  
            if rem:  
                with open(CONFIG_FILE, "w") as f:  
                    json.dump({"user": u_l}, f)  
            st.rerun()  
        else:  
            st.error("Hatalı!")  
    ...
    if st.button("HESAP OLUŞTUR"):  
        if nu and np and nu not in st.session_state.db:  
            st.session_state.db[nu] = {  
                ...
                'xp': 0,  
                'level': 1,  
                'data': pd.DataFrame(columns=['Gün','Görev','Hedef','Yapılan'])  
            }  
            veritabanini_kaydet(st.session_state.db)  
            st.success("Tamam! Giriş yapın.")  
...
h_df = pd.DataFrame(  
    u_info.get('habits', []),  
    columns=["Alışkanlık", "Pzt", "Sal", "Çar", "Per", "Cum", "Cmt", "Paz"]  
)  
...
if not h_df.equals(edited):  
    u_info['habits'] = edited.to_dict(orient='records')  
    veritabanini_kaydet(st.session_state.db)
...
with t2:  
    ...
    m_gano = st.number_input(  
        "Mevcut GNO",  
        0.0,  
        4.0,  
        value=float(u_info['mevcut_gano'])  
    )  
...
if st.button("📊 HAFTALIK ANALİZ RAPORU OLUŞTUR"):  
    res = genai.GenerativeModel(  
        ...
        f"Analiz et: {u_info['data'].to_string()}"  
    ).text  
    st.markdown(res)  
...
if st.button("TEMA UYGULA"):  
    u_info['tema_rengi'] = new_theme  
    veritabanini_kaydet(st.session_state.db)  
    st.rerun() Bu mu full kod